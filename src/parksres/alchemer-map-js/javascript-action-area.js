...
function updateAlchemerField() {
  ...
  alchemerField.value = userSelections[0] || "";
...
function setSelected(entry, selected) {
  ...
  if (selected) {
    // one area at a time - drop whatever was selected before
    parkRegistry.forEach(function (other) {
    ...
    userSelections = [entry.park.name];
  } else {
    userSelections = [];
  }
...
function buildMap() {
  ...
  const parkData = window.azureParksData;   // every area is selectable
