...
function updatePinField() {
  const pinField = document.getElementById(ALCHEMER_PIN_FIELD_ID);
  if (!pinField) {
    console.warn("[PARKMAP] Pin field not found:", ALCHEMER_PIN_FIELD_ID);
    return;
  }
  pinField.value = userPins.map(function (p) {
...
// Read a marker's current position as plain numbers (works before/after a drag).
function coordsOf(marker) {
...
function renderPins() {
  ...
  userPins.forEach(function (pin) {
    ...
    remove.setAttribute("aria-label", "Remove pin " + pin.id);
...
function injectMapsScriptOnce() {
  return new Promise(function (resolve, reject) {
    ...
    script.src = "https://maps.googleapis.com/maps/api/js?key=" + GOOGLE_MAPS_KEY +
      "&v=weekly&libraries=places,marker&callback=__parkMapMapsReady";
    script.async = true;
    script.defer = true;
